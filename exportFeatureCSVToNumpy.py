...
#             '-' + data_type + '.npy')
fileName = '20180802-095434_390-MatchedToMP4.csv'
...
seq_length = 30
metrics = readCSV(os.path.join(os.getcwd(), 'data', 'train', fileName),0)
metricsTrimmed = np.hstack((metrics[:,2:8] , metrics[:, 9:]))

annotation_file = 'ourdata_ordinalRegression.json'
labels = None
classnames = {}
...
with open(os.path.join(os.getcwd(),'data', 'data_file_ordinal_logistic_regression.csv'), 'w') as fout:
